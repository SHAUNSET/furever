...
export default function Registration() {
  ...
  return (
    ...
    >
      {/* Header: logo left, log-in link right */}
      <div className="flex items-center justify-between px-6 sm:px-10 py-6 sm:py-8">
        ...
        >
          <span className="hidden sm:inline text-[#8A8FB0]">Already a member?</span>
          Log in
        </a>
      </div>
          ...
          <div className="bg-white rounded-3xl border border-[#EFEFF6] shadow-[0_10px_40px_-12px_rgba(20,23,46,0.12)] p-7 sm:p-14">
            {status === "success" ? (
              <div className="text-center py-8">
                ...
                <p className="text-[#8A8FB0] text-lg">Your account is ready.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                {/* Name */}
                <div className="mb-6">
                  <label htmlFor="name" className="block text-lg font-medium text-[#14172E] mb-2.5">
                    Name
                  </label>
                  <div className="relative">
                    <User size={22} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#9DA1C4]" />
                    <input
                      id="name"
                      type="text"
                      value={form.name}
                      ...
                      className="w-full pl-14 pr-5 py-4.5 rounded-xl border border-[#E7E7F3] bg-white text-lg text-[#14172E] placeholder:text-[#B0B3CC] outline-none focus:border-[#3B4CE0] focus:ring-2 focus:ring-[#3B4CE0]/15 transition-all"
                      style={{ paddingTop: "1.1rem", paddingBottom: "1.1rem" }}
                    />
                  </div>
                  {touched.name && !valid.name && (
                    <p className="text-[#FF7A5C] text-base mt-2">Name needs at least 2 characters.</p>
                  )}
                </div>
